{
    ...
    switch (idx) {
      ...
      case 0:
        ...
        if (tmr % 65 < 60) {
          ...
          fText("タッチ操作：タッチで移動", 600, 600, 20, "#5e5e5e");
          fText("Auto Missileボタン：自動発射の有無を切り替える", 600, 650, 20, "#5e5e5e");
        }
        // スペースキーorタップでゲーム開始
        if (key[32] > 0 || tapC > 0) {
          initSShip();
  ...
  function moveSShip() {
    ...
    // ----- 自機を描画 ------
    if (weapon > 0 && muteki % 2 == 0) {
      //weaponを所持するならハチワレを描画
